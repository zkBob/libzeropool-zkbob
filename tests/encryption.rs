...
fn test_encryption() {
    ...
    let sender_kappa = derive_key_kappa(sender_eta);
    ...
    let ciphertext = cipher::encrypt(&(0..32).map(|_| rng.gen()).collect::<Vec<_>>(), &sender_kappa, account, &note, &*POOL_PARAMS);

    let result_out = cipher::decrypt_out(sender_eta, &sender_kappa, &ciphertext, &*POOL_PARAMS);
